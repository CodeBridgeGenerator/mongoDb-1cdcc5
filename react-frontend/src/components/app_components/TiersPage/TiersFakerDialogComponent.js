...
import tiersFakerFactory from "./tiersFakerFactory";
...
const TiersFakerDialogComponent = (props) => {
    ...
    const onRun = async () => {
        let fakeData = tiersFakerFactory(props.user,fakerCount);

        setLoading(true);
        const promises = fakeData.map((elem) => client.service("tiers").create(elem));
...
export default connect(mapState, mapDispatch)(TiersFakerDialogComponent);
