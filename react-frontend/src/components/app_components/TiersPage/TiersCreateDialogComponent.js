...
const TiersCreateDialogComponent = (props) => {
    ...
    const onSave = async () => {
        if(!validate()) return;
        let _data = {
            tierName: _entity?.tierName,storage: _entity?.storage,ram: _entity?.ram,vCpu: _entity?.vCpu,basePrice: _entity?.basePrice,markedUpPrice: _entity?.markedUpPrice,finalPrice: _entity?.finalPrice,
        ...
        const result = await client.service("tiers").create(_data);
        props.onHide();
        props.alert({ type: "success", title: "Create info", message: "Info Tiers created successfully" });
        props.onCreateResult(result);
        } catch (error) {
            console.debug("error", error);
            setError(getSchemaValidationErrorsStrings(error) || "Failed to create");
            props.alert({ type: "error", title: "Create", message: "Failed to create in Tiers" });
        }
        setLoading(false);
    };
    ...
    return (
        <Dialog header="Create Tiers" visible={props.show} closable={false} onHide={props.onHide} modal style={{ width: "40vw" }} className="min-w-max scalein animation-ease-in-out animation-duration-1000" footer={renderFooter()} resizable={false}>
            <div className="grid p-fluid overflow-y-auto"
            style={{ maxWidth: "55vw" }} role="tiers-create-dialog-component">
...
export default connect(mapState, mapDispatch)(TiersCreateDialogComponent);
