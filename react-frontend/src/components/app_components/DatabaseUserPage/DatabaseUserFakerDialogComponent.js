...
import databaseUserFakerFactory from "./databaseUserFakerFactory";
...
const DatabaseUserFakerDialogComponent = (props) => {
    ...
    const onRun = async () => {
        let fakeData = databaseUserFakerFactory(props.user,fakerCount,projectIdIds);

        setLoading(true);
        const promises = fakeData.map((elem) => client.service("databaseUser").create(elem));
...
export default connect(mapState, mapDispatch)(DatabaseUserFakerDialogComponent);
