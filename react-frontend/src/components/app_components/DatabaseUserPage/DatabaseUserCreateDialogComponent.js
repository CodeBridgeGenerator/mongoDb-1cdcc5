...
const DatabaseUserCreateDialogComponent = (props) => {
    ...
    const onSave = async () => {
        if(!validate()) return;
        let _data = {
            userId: _entity?.userId,projectId: _entity?.projectId,mongodbUser: _entity?.mongodbUser,mongodbPassword: _entity?.mongodbPassword,role: _entity?.role,
        ...
        props.onHide();
        props.alert({ type: "success", title: "Create info", message: "Info Database User created successfully" });
