...
import projectClustersFakerFactory from "./projectClustersFakerFactory";
...
const ProjectClustersFakerDialogComponent = (props) => {
    const [fakerCount, setFakerCount] = useState(1);
    const [errors, setErrors] = useState([]);
    const [loading, setLoading] = useState(false);
    const [projectIdIds, setprojectIdIds] = useState();
    ...
    const onRun = async () => {
        let fakeData = projectClustersFakerFactory(props.user,fakerCount,projectIdIds);

        setLoading(true);
        const promises = fakeData.map((elem) => client.service("projectClusters").create(elem));
...
export default connect(mapState, mapDispatch)(ProjectClustersFakerDialogComponent);
