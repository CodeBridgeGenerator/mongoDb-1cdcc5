...
import ipAddressFakerFactory from "./ipAddressFakerFactory";
...
const IpAddressFakerDialogComponent = (props) => {
    ...
    const onRun = async () => {
        let fakeData = ipAddressFakerFactory(props.user,fakerCount,projectIdIds);

        setLoading(true);
        const promises = fakeData.map((elem) => client.service("ipAddress").create(elem));
...
export default connect(mapState, mapDispatch)(IpAddressFakerDialogComponent);
