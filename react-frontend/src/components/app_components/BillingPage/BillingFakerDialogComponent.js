...
import billingFakerFactory from "./billingFakerFactory";
...
const BillingFakerDialogComponent = (props) => {
    ...
    const onRun = async () => {
        let fakeData = billingFakerFactory(props.user,fakerCount,projectIdIds);

        setLoading(true);
        const promises = fakeData.map((elem) => client.service("billing").create(elem));
...
export default connect(mapState, mapDispatch)(BillingFakerDialogComponent);
