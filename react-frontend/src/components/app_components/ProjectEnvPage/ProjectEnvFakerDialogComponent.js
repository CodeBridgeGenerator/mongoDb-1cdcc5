...
import projectEnvFakerFactory from "./projectEnvFakerFactory";
...
const ProjectEnvFakerDialogComponent = (props) => {
    ...
    const onRun = async () => {
        let fakeData = projectEnvFakerFactory(props.user,fakerCount);

        setLoading(true);
        const promises = fakeData.map((elem) => client.service("projectEnv").create(elem));
...
export default connect(mapState, mapDispatch)(ProjectEnvFakerDialogComponent);
